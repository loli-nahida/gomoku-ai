...
(function () {
  ...
  async function loadModel() {
    try {
      ...
      // 尝试从 model-weights.json 加载预训练权重
      try {
        const response = await fetch('model-weights.json');
        if (response.ok) {
          const data = await response.json();
          ai.network.importWeights(JSON.stringify(data));
          console.log('Model loaded from model-weights.json');
          return;
        }
      } catch (fetchErr) {
        console.log('model-weights.json not available, using random weights');
      }
    } catch (e) {
